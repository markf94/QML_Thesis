...
\newline
When approximating a unitary gate $U$ with a gate sequence yielding another unitary gate $U_{approx}$ it is important to quantify how 'close' the action of $U_{approx}$ is to $U$. The Fowler distance makes use of the matrix representations of $U$ and $U_{approx}$ and is defined by \citeA{booth2012quantum} as:
\begin{equation}
d(U,U_{approx}) = \sqrt{\frac{2-\mid tr(U\cdot U_{approx}^\dagger)\mid}{2}}
\end{equation}

In contrast to $U$, $U_{approx}$ might introduce a shift in the global phase of the quantum state it is acting on. However, since the global phase of a quantum state is immeasurable in experiment it can be neglected. As a result, the Fowler distance is defined in such a way that possible global phase differences are ignored.
\end{redbox}
...
In summary, in this particular classification problem the constant complexity of the algorithm is dominated by the complexity of so-called quantum compiling: preparing the initial quantum state using the universal gate set at hand.
