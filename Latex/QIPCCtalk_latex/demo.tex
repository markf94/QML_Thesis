...
\begin{equation}
p(\ket{y^m} = \ket{0})= \sum_{m \mid y^m=0} 1 - \frac{1}{4} \mid \tilde{x} - x^m \mid ^2
\end{equation}
	\metroset{block=fill}
...
After a long time, we expect this probability to go to 0.5 since it becomes more and more likely that something caused the qubit to move towards the $\ket{0}$ or $\ket{1}$.
\end{block}
